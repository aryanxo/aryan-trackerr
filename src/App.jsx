...
function scheduleNotifications(tasks){
  ...
  tasks.filter(t=>!t.done&&t.due&&t.notifyHrs>0).forEach(task=>{
    ...
    if(notifyAt>now&&!fired[key]){
      const delay=Math.min(notifyAt-now,2147483647);
      setTimeout(()=>{
        new Notification("Task Reminder ⏰",{
          body:`"${task.title}" — due ${task.notifyHrs>=24?`in ${task.notifyHrs/24} day(s)`:`in ${task.notifyHrs}h`}`,
          tag:key,
        });
        const f=JSON.parse(localStorage.getItem("notif_fired")||"{}");
        f[key]=true;
...
const glass=(T,r=20,extra={})=>({background:T.g1,backdropFilter:"blur(40px) saturate(200%)",WebkitBackdropFilter:"blur(40px) saturate(200%)",borderRadius:r,border:`0.5px solid ${T.b1}`,borderTop:`0.5px solid ${T.bTop}`,boxShadow:T.shadow+(T.dark?", inset 0 0.5px 0 rgba(255,255,255,0.10)":", inset 0 0.5px 0 rgba(255,255,255,0.80)"),...extra});
...
// ── Avatar ────────────────────────────────────────────────────────────────────
function Av({name,size=22,T}){const c=pc(name,T.dark);return(<div style={{width:size,height:size,borderRadius:size,background:c.dim,border:`0.5px solid ${c.glow}`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><span style={{fontSize:size*.37,fontWeight:800,color:c.accent,letterSpacing:-0.5}}>{initials(name)}</span></div>);}
    ...
    <div style={{display:"flex",alignItems:"center",gap:12,padding:"13px 16px",borderBottom:`0.5px solid ${T.sep}`,opacity:exiting?0:1,transform:exiting?"translateX(36px) scale(0.96)":"translateX(0) scale(1)",transition:"opacity 0.3s ease, transform 0.3s cubic-bezier(0.4,0,1,1)"}}>
      ...
      <div style={{flex:1,minWidth:0}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
          ...
          {task.notifyHrs>0&&task.due&&!task.done&&<span style={{fontSize:10,color:T.textFaint,letterSpacing:0.1}}>🔔 {task.notifyHrs}h before</span>}
        </div>
      </div>
  ...
  return(
    <div style={{...glass(T,20),overflow:"hidden",animation:`slideUp 0.42s ${idx*0.09}s both`}}>
      <button onClick={()=>setOpen(o=>!o)} style={{width:"100%",padding:"14px 16px 13px",display:"flex",alignItems:"center",gap:10,background:"transparent",border:"none",cursor:"pointer",textAlign:"left"}}>
        ...
        <span style={{fontSize:11,fontWeight:700,padding:"3px 11px",borderRadius:20,color:allDone?T.green:c.accent,background:allDone?T.statBg(T.green):T.statBg(c.accent),border:`0.5px solid ${allDone?T.statBorder(T.green):T.statBorder(c.accent)}`}}>{allDone?"Done ✓":`${pending} left`}</span>
        <svg width="12" height="12" viewBox="0 0 12 12" fill="none" style={{transition:"transform 0.28s cubic-bezier(0.4,0,0.2,1)",transform:open?"rotate(0)":"rotate(-90deg)",color:T.textFaint,flexShrink:0}}><path d="M2 4.5L6 8.5L10 4.5" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>
      </button>
...
function NotifBanner({T,onGrant}){
  ...
  return(
    <div style={{...glass(T,16),margin:"0 0 12px",padding:"14px 16px",display:"flex",alignItems:"center",gap:12,border:`0.5px solid ${T.orange}55`}}>
...
function AddSheet({onAdd,onClose,T,projectSuggestions,spocSuggestions}){
  ...
  const label=(txt)=>(<div style={{fontSize:11,fontWeight:700,color:T.textFaint,letterSpacing:0.8,marginBottom:7}}>{txt}</div>);
      ...
      <div style={{position:"relative",width:"100%",maxWidth:430,background:T.dark?"rgba(18,18,20,0.97)":"rgba(252,252,254,0.97)",backdropFilter:"blur(60px) saturate(200%)",WebkitBackdropFilter:"blur(60px) saturate(200%)",borderRadius:"30px 30px 0 0",borderTop:`0.5px solid ${T.bTop}`,borderLeft:`0.5px solid ${T.b1}`,borderRight:`0.5px solid ${T.b1}`,boxShadow:T.dark?"0 -12px 60px rgba(0,0,0,0.7)":"0 -8px 40px rgba(0,0,0,0.12)",animation:"sheetUp 0.38s cubic-bezier(0.34,1.1,0.64,1) both",paddingBottom:44,maxHeight:"92vh",overflowY:"auto"}}>
        ...
        <div style={{padding:"18px 20px 0"}}>
          ...
          <div style={{marginBottom:14}}>
            <AutoInput value={form.project} onChange={v=>setForm(f=>({...f,project:v}))} placeholder="📁  e.g. Brand Refresh" suggestions={projectSuggestions} T={T} style={inp()}/>
          </div>

          {label("SPOC / PERSON")}
          <div style={{marginBottom:14}}>
            <AutoInput value={form.spoc} onChange={v=>setForm(f=>({...f,spoc:v}))} placeholder="👤  e.g. Anika M." suggestions={spocSuggestions} T={T} style={inp()}/>
          </div>
...
export default function App(){
  ...
  useEffect(()=>{
    try{const saved=localStorage.getItem("taskapp_v1");if(saved){const s=JSON.parse(saved);setTasks(s.tasks||SAMPLE);setDark(s.dark!==undefined?s.dark:true);}else setTasks(SAMPLE);}catch{setTasks(SAMPLE);}
    setLoaded(true);
  },[]);
