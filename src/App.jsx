...
const makeTheme = (dark) => dark ? {
  dark:true, bg:"#000000", bgPage:"#000000",
  g1:"rgba(28,28,30,0.78)", g2:"rgba(44,44,46,0.68)", g3:"rgba(58,58,60,0.55)",
  ...
  dark:false, bg:"#F2F2F7", bgPage:"#E8E8ED",
  g1:"rgba(255,255,255,0.85)", g2:"rgba(248,248,253,0.80)", g3:"rgba(235,235,240,0.90)",
  ...
  return(
    <div style={{...glass(T,20),overflow:"hidden",animation:`slideUp 0.42s ${idx*0.09}s both`}}>
      <button onClick={()=>setOpen(o=>!o)} style={{width:"100%",padding:"14px 16px 13px",display:"flex",alignItems:"center",gap:10,background:"transparent",border:"none",cursor:"pointer",textAlign:"left"}}>
        ...
        <span style={{fontSize:11,fontWeight:700,padding:"3px 11px",borderRadius:20,color:allDone?T.green:c.accent,background:allDone?T.statBg(T.green):T.statBg(c.accent),border:`0.5px solid ${allDone?T.statBorder(T.green):T.statBorder(c.accent)}`}}>
          {allDone?"Done ✓":`${pending} left`}
        </span>
        <svg width="12" height="12" viewBox="0 0 12 12" fill="none" style={{transition:"transform 0.28s cubic-bezier(0.4,0,0.2,1)",transform:open?"rotate(0)":"rotate(-90deg)",color:T.textFaint,flexShrink:0}}>
...
function AddSheet({onAdd,onClose,T}){
  ...
  const inp=(override={})=>({width:"100%",background:T.g3,border:`0.5px solid ${T.b1}`,borderRadius:14,padding:"13px 14px",fontSize:14,fontFamily:"inherit",color:T.text,outline:"none",letterSpacing:-0.15,transition:"border 0.18s, box-shadow 0.18s",...override});
    ...
    <div style={{position:"fixed",inset:0,zIndex:300,display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
      ...
      <div style={{position:"relative",width:"100%",maxWidth:430,background:T.dark?"rgba(18,18,20,0.96)":"rgba(252,252,254,0.96)",backdropFilter:"blur(60px) saturate(200%)",WebkitBackdropFilter:"blur(60px) saturate(200%)",borderRadius:"30px 30px 0 0",borderTop:`0.5px solid ${T.bTop}`,borderLeft:`0.5px solid ${T.b1}`,borderRight:`0.5px solid ${T.b1}`,boxShadow:T.dark?"0 -12px 60px rgba(0,0,0,0.7)":"0 -8px 40px rgba(0,0,0,0.12)",animation:"sheetUp 0.38s cubic-bezier(0.34,1.1,0.64,1) both",paddingBottom:36}}>
        ...
        <div style={{padding:"18px 20px 0"}}>
          ...
          <textarea ref={titleRef} value={form.title} onChange={e=>setForm(f=>({...f,title:e.target.value}))} placeholder="What needs to get done?" rows={2} style={{...inp({fontSize:16,resize:"none",lineHeight:1.5,marginBottom:10,...(form.title?T.inputFocus(T.blue):{})}),}}/>
...
function ThemeToggle({dark,onToggle,T}){
  return(
    <button onClick={onToggle} style={{width:34,height:34,borderRadius:34,background:T.toggleBg,border:`0.5px solid ${T.toggleBorder}`,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",transition:"all 0.25s cubic-bezier(0.34,1.3,0.64,1)",color:T.textSub,flexShrink:0}} onMouseDown={e=>e.currentTarget.style.transform="scale(0.88)"} onMouseUp={e=>e.currentTarget.style.transform="scale(1)"} onMouseLeave={e=>e.currentTarget.style.transform="scale(1)"}>
      {dark
...
export default function App(){
  ...
  // ── localStorage (works everywhere outside Claude) ──
  useEffect(()=>{
    try{
      const saved=localStorage.getItem("taskapp_v1");
      if(saved){const s=JSON.parse(saved);setTasks(s.tasks||SAMPLE);setDark(s.dark!==undefined?s.dark:true);}
      else setTasks(SAMPLE);
    }catch{setTasks(SAMPLE);}
    setLoaded(true);
  },[]);

  useEffect(()=>{
    if(loaded) try{localStorage.setItem("taskapp_v1",JSON.stringify({tasks,dark}));}catch{}
  },[tasks,dark,loaded]);
      ...
      <div style={{width:"100%",maxWidth:430,minHeight:"100vh",background:T.bg,fontFamily:"'Outfit',system-ui,sans-serif",color:T.text,position:"relative",overflow:"hidden",transition:"background 0.4s ease"}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            ...
            <div style={{display:"flex",gap:6,alignItems:"center",color:dark?"rgba(255,255,255,0.85)":"rgba(0,0,0,0.75)"}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:1}}>
                <div style={{width:27,height:13,borderRadius:4,border:`1.5px solid ${dark?"rgba(255,255,255,0.45)":"rgba(0,0,0,0.35)"}`,padding:"1.5px 2px",display:"flex",alignItems:"center",gap:1}}>
        ...
        <div style={{padding:"20px 24px 0",position:"relative",zIndex:10}}>
          ...
          <div style={{height:3,background:T.progressBg,borderRadius:3,marginBottom:18}}>
            <div style={{height:"100%",width:`${pct}%`,background:`linear-gradient(90deg,${T.blue},${T.green})`,borderRadius:3,transition:"width 0.7s ease",boxShadow:dark?`0 0 10px ${T.blue}88`:"none"}}/>
          </div>
        ...
        <div style={{padding:"14px 20px 130px",display:"flex",flexDirection:"column",gap:12,position:"relative",zIndex:10}}>
          ...
          {groups.map(([name,items],i)=><GroupCard key={name} name={name} tasks={items} view={view} onToggle={toggle} onDelete={remove} idx={i} T={T}/>)}
        </div>

        {/* Tab bar */}
        <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:430,background:T.tabBg,backdropFilter:"blur(50px) saturate(200%)",WebkitBackdropFilter:"blur(50px) saturate(200%)",borderTop:`0.5px solid ${T.tabBorder}`,padding:"10px 36px 30px",display:"flex",justifyContent:"space-between",alignItems:"center",zIndex:50,transition:"background 0.4s ease, border-color 0.4s ease"}}>
          ...
          <button onClick={()=>setShowAdd(true)} style={{width:56,height:56,borderRadius:56,background:T.blue,border:"none",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",boxShadow:`0 4px 22px ${T.blue}66`,transition:"transform 0.2s cubic-bezier(0.34,1.56,0.64,1)",marginTop:-30}} onMouseDown={e=>e.currentTarget.style.transform="scale(0.87)"} onMouseUp={e=>e.currentTarget.style.transform="scale(1)"} onMouseLeave={e=>e.currentTarget.style.transform="scale(1)"}>
